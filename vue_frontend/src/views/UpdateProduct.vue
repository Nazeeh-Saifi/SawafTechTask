<template>
  <b-container class="update-product">
    <b-row>
      <b-col>
        <b-card header-tag="header" footer-tag="footer" align="left">
          <template #header>
            <div>
              <h6 class="mb-0">update Product {{ id }}:</h6>
            </div>
          </template>
...
export default {
  ...
  computed: {
    state1() {
      return this.form.ProductName.length >= 4;
    },
    invalidFeedback1() {
      if (this.form.ProductName.length > 0) {
  ...
  methods: {
    ...
    getProduct() {
      let self = this;
      axios
        .get(this.$apiAdress + "/api/products/" + this.id)
        .then((response) => {
          console.log(response);
          self.form.ProductName = response.data.product.ProductName;
          self.form.UnitPrice = response.data.product.UnitPrice;
          self.form.SupplierId = response.data.product.SupplierId;
        })
        .catch((error) => {
          console.log(error);
        });
