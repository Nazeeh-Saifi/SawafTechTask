...
export default {
  ...
  methods: {
    ...
    store() {
      ...
      //   let self = this;
      axios
        .post(this.$apiAdress + "/api/orders/" + this.id, formData)
    ...
    getOrder() {
      let self = this;
      axios
        .get(this.$apiAdress + "/api/orders/" + this.id)
        .then(function (response) {
          console.log(response);
          self.prevSelectedProducts = response.data.order.products;
          self.OrderDate = response.data.order.OrderDate;
          self.CustomerId = response.data.order.CustomerId;
          response.data.order.products.forEach((elem) => {
            Vue.set(self.SelectedProducts, elem.id, elem.pivot.Quantity);
            // self.SelectedProducts[elem.id] = elem.pivot.Quantity;
          });
          /* .map((obj) => ({
            value: obj.id,
            keyword: obj.ProductName,
